 ...
 */
const HOME = process.env.FINEDU_MCP_HOME
  ? path.resolve(process.env.FINEDU_MCP_HOME)
  : path.join(os.homedir(), ".finedu-broker-mcp");
...
export const guardrails = {
  get maxPerOrderEur(): number {
    return Number(process.env.FINEDU_MAX_PER_ORDER_EUR ?? 250);
  },
  get maxPerMonthEur(): number {
    return Number(process.env.FINEDU_MAX_PER_MONTH_EUR ?? 1000);
  },
};
