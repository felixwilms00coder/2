...
[`/agent`](../src/app/agent) feature (see [`docs/automatisering.md`](../docs/automatisering.md)),
...
- **Runs on your machine only.** You start this process yourself; it is not
  hosted by FinEdu and FinEdu never sees it run.
...
[`docs/automatisering.md`](../docs/automatisering.md) for why that
distinction matters under Belgian/EU financial regulation (MiFID II) and
...
4. **Hard euro guardrails**, checked at both preview and confirm time:
   `FINEDU_MAX_PER_ORDER_EUR` (default 250) and `FINEDU_MAX_PER_MONTH_EUR`
...
5. **Everything is logged locally.** Every preview, fill, rejection,
   ...
   `~/.finedu-broker-mcp/orders.log.jsonl` — nothing is sent to FinEdu.
...
left out rather than built against DEGIRO's private app API — the same
treatment `docs/automatisering.md` already gives Bolero. Revisit if DEGIRO
ever ships an official one.
  ...
  "mcpServers": {
    "finedu-broker": {
      ...
      "env": {
        ...
        "SAXO_MODE": "sim",
        "SAXO_TOKEN": "...",
        "FINEDU_MAX_PER_ORDER_EUR": "250",
        "FINEDU_MAX_PER_MONTH_EUR": "1000"
      }
    }
  }
}
...
| `get_order_history` | No | Local audit log from `~/.finedu-broker-mcp`. |
