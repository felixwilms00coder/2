export type Frequency = "wekelijks" | "tweewekelijks" | "maandelijks";

/**
 * A rule the user writes themselves. FinEdu never creates, suggests or
 ...
 * user's own browser against the user's own account credentials — FinEdu
 * has no server in this path and never sees an order.
 */
...
export const EMPTY_AGENT_STATE: AgentState = {
  settings: { armed: false, brokerId: "simulatie" },
  rules: [],
  log: [],
};
