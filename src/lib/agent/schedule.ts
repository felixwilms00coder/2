...
export function nextRunDate(rule: Rule, from: Date = new Date()): Date {
  ...
  if (rule.frequency === "wekelijks" || rule.frequency === "tweewekelijks") {
    const step = rule.frequency === "wekelijks" ? 7 : 14;
...
/** Euro already committed for a rule in the calendar month of `now`. */
export function spentThisMonth(
  ...
  return log
    .filter((e) => {
      ...
      if (e.status !== "uitgevoerd" && e.status !== "gesimuleerd") return false;
...
export function checkGuardrails(
  ...
  if (rule.amount <= 0) {
    return { ok: false, reason: "Bedrag is 0." };
  }
  if (rule.amount > rule.maxPerOrder) {
    return {
      ok: false,
      reason: `Bedrag ${rule.amount} € ligt boven je limiet van ${rule.maxPerOrder} € per order.`,
    };
  }
  ...
  if (spent + rule.amount > rule.maxPerMonth) {
    return {
      ok: false,
      reason: `Maandlimiet bereikt: al ${spent} € van ${rule.maxPerMonth} € gebruikt deze maand.`,
    };
  }
  return { ok: true };
}
