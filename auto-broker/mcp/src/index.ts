 ...
 * Auto Broker server anywhere in this path: it is the same "software, not a
 * service" posture as the web app's own /agent feature
 * (src/lib/agent/brokers), just reachable from an MCP client instead of a
 * browser. See docs/regulatory.md for why that architecture matters.
 *
 * Safety model:
 ...
 *    a local, append-only audit log (~/.auto-broker-mcp by default).
 */

const server = new McpServer({ name: "auto-broker-mcp", version: "0.1.0" });
