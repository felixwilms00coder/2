...
export function checkGuardrails(req: OrderRequest, brokerId: string): GuardrailResult {
  ...
  if (req.amountEur > limits.maxPerOrderEur) {
    return {
      ok: false,
      reason: `Amount ${req.amountEur} EUR exceeds the per-order limit of ${limits.maxPerOrderEur} EUR (set AUTOBROKER_MAX_PER_ORDER_EUR to change it).`,
    };
  }
  ...
  if (spent + req.amountEur > limits.maxPerMonthEur) {
    return {
      ok: false,
      reason: `Monthly limit reached: already ${spent} EUR of ${limits.maxPerMonthEur} EUR used this month on "${brokerId}" (set AUTOBROKER_MAX_PER_MONTH_EUR to change it).`,
    };
  }
  return { ok: true };
}
