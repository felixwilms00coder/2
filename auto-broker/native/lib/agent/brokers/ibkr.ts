...
/**
 * Interactive Brokers via the Client Portal Web API — run entirely on-device
 * against the user's own, locally-running IBKR Client Portal Gateway (the
 * same Java process IBKR ships for this API). There is no Auto Broker
 * backend in this path: every call goes straight from this app to
 * https://localhost:5000, exactly like the Saxo adapter goes straight to
 * Saxo.
 *
 * ⚠️ "localhost" means something different on a phone than on a laptop: it
 * is the phone itself, not the computer running the gateway. This adapter
 * will simply fail to connect on mobile unless `ibkrStoreGatewayUrl()` is
 * pointed at that computer's LAN/VPN address instead of the default — the
 * same limitation already documented for the Capacitor build in
 * mobile/README.md. Nothing here works around that; it's a real deployment
 * decision, not a bug.
 *
 * Setup the user has to do once, outside this app:
 *  1. Download and run IBKR's Client Portal Gateway, on a machine reachable
 *     from the phone (i.e. not the phone itself).
 *  2. Open https://<that machine>:5000 in a browser and log in there with
 *     their own IBKR credentials — that browser session holds the actual
 *     login; this adapter never sees an IBKR password.
 *  3. Since the gateway's certificate is self-signed, the OS/WebView will
 *     warn about it once; accepting that is IBKR's own standard setup step
 *     for this API, not something specific to Auto Broker.
 *
...
async function resolveAccountId(): Promise<string> {
  ...
  if (!first) {
    throw new Error(
      "No accounts found. Log in at https://localhost:5000 first.",
    );
  }
...
export async function ibkrCheckConnection(): Promise<string> {
  try {
    ...
    if (!status.authenticated) {
      return "Gateway is reachable but not logged in — open https://localhost:5000 and log in.";
    }
    const accountId = await resolveAccountId();
    knownLive = !looksLikePaperAccountId(accountId);
    return knownLive
      ? `Connected. Account ${accountId} does not look like a paper account — double-check before you rely on this.`
      : `Connected. Account ${accountId} looks like a paper account.`;
  } catch (err) {
    return `Not reachable at ${ibkrGatewayUrl()}: ${err instanceof Error ? err.message : String(err)}`;
  }
}
...
export function createIbkrBroker(): BrokerAdapter {
  return {
    id: "ibkr",
    label: "Interactive Brokers (your own connection)",
    get isLive() {
      return knownLive;
    },
    ...
    connectionSummary: () => {
      ...
      return knownLive
        ? "Connected to the IBKR Client Portal Gateway — the account does not look like a paper account. Orders may cost real money."
        : "Connected to the IBKR Client Portal Gateway (or not checked yet). Log in at https://localhost:5000 if orders fail.";
    },
    ...
    async placeOrder(req: OrderRequest): Promise<PlacedOrder> {
      ...
      if (quantity < 1) {
        throw new Error(
          `Amount ${req.amountEur} is too small for one unit (price ${price}).`,
        );
      }
      ...
      // it here is safe because the user already confirmed via Auto
      // Broker's own confirm-before-send step (autoConfirm / the "Send"
      // button in agent-dashboard.tsx) before this function was ever
      // called — this is IBKR's own internal dialog, not a second human gate.
      let guard = 0;
      ...
      if (!orderId) {
        throw new Error("IBKR did not return an order id — check the gateway.");
      }
